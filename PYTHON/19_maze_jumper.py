from data.data_19_maze_jumper import puzzle_input
...
def mazeJumper(arr):
  arr     = arr[:] # shallow copy to avoid change passed-by-value variable
  arr_len = len(arr)
  cur_pnt = 0
  moves   = 0
  while cur_pnt < arr_len:
    jump          = arr[cur_pnt]
    arr[cur_pnt] += 1
    cur_pnt      += jump
    moves        += 1
